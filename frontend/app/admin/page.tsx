...
import { ApiError, api, type CrowdMetrics, type TicketTier, type Visitor, type VisitorFilter, type VisitorMetrics } from '@/lib/api';
import { useStaffAuth } from '@/lib/use-staff-auth';
import CrudPanel, { type CrudConfig } from '@/components/crud-panel';
...
export default function AdminPage() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    e.preventDefault();
    // The backend validates the password and mints a short-lived JWT; the
    // password is never compared in the browser.
    const ok = await auth.signIn(password);
    if (ok) setPassword('');
  };

  const signOut = () => { auth.signOut(); setPassword(''); };

  /** Surface an API failure, bouncing to the sign-in screen if the token died. */
  const { handleAuthFailure } = auth;
  const reportError = useCallback((err: unknown, fallback: string) => {
    if (handleAuthFailure(err)) return;
    setDataError(err instanceof ApiError ? err.message : fallback);
  }, [handleAuthFailure]);

  // All five counts plus live occupancy now arrive in a single request.
  const fetchMetrics = useCallback(async () => {
    try {
      const { metrics: next, crowd: nextCrowd } = await api.admin.metrics();
      setMetrics(next);
      setCrowd(nextCrowd);
      setCapacityInput(nextCrowd.capacity);
    } catch (err) {
      reportError(err, 'Could not load dashboard metrics.');
    }
  }, [reportError]);
  ...
  const fetchVisitors = useCallback(async () => {
    setLoading(true);
    try {
      const result = await api.admin.visitors({ filter, search: search.trim(), page, pageSize });
      setVisitors(result.visitors);
      setTotalVisitors(result.total);
    } catch (err) {
      reportError(err, 'Could not load visitors.');
    }
    setLoading(false);
  }, [page, filter, search, reportError]);

  const fetchTiers = useCallback(async () => {
    setTierLoading(true);
    try {
      const { tiers: rows } = await api.admin.ticketTiers();
      setTiers(rows);
    } catch (err) {
      reportError(err, 'Could not load ticket tiers.');
    } finally {
  ...
  const fetchEventDate = useCallback(async () => {
    try {
      const { settings } = await api.admin.eventSettings();
      if (settings?.event_date) setEventDate(settings.event_date);
      if (settings?.event_end_date) setEventEndDate(settings.event_end_date);
      if (settings?.ground_capacity) setCapacityInput(settings.ground_capacity);
    } catch (err) {
      reportError(err, 'Could not load event settings.');
    }
  }, [reportError]);
  ...
  useEffect(() => {
    ...
    fetchEventDate();
    fetchTiers();
  }, [authed, fetchMetrics, fetchEventDate, fetchTiers]);

  useEffect(() => {
    if (!authed) return;
    const timer = setTimeout(() => { fetchVisitors(); }, search ? 300 : 0);
    return () => clearTimeout(timer);
  }, [authed, fetchVisitors, search]);

  /** Replace one row in the loaded page without refetching the whole list. */
  const patchVisitorRow = (id: string, changes: Partial<Visitor>) =>
    setVisitors((prev) => prev.map((x) => (x.id === id ? { ...x, ...changes } : x)));

  const markPaid = async (v: Visitor) => {
    setActionLoading(v.id); setDataError(null);
    try {
      const { visitor } = await api.admin.markPaid(v.id);
      patchVisitorRow(v.id, visitor);
      fetchMetrics();
    } catch (err) {
      reportError(err, 'Could not mark this visitor as paid.');
    } finally {
  ...
  const allowEntry = async (v: Visitor) => {
    setActionLoading(v.id); setDataError(null);
    try {
      // Shares the capacity-checked gate function with the scanner.
      const { visitor, crowd: nextCrowd } = await api.admin.allowEntry(v.id);
      if (visitor) patchVisitorRow(v.id, visitor);
      setCrowd(nextCrowd);
      fetchMetrics();
    } catch (err) {
      reportError(err, 'Could not admit this visitor.');
    } finally {
      setActionLoading(null);
    }
  };
  ...
  const markExited = async (v: Visitor) => {
    setActionLoading(v.id); setDataError(null);
    try {
      const { visitor, crowd: nextCrowd } = await api.admin.markExited(v.id);
      if (visitor) patchVisitorRow(v.id, visitor);
      setCrowd(nextCrowd);
      fetchMetrics();
    } catch (err) {
      reportError(err, 'Could not mark this visitor as exited.');
    } finally {
      setActionLoading(null);
    }
  };
  ...
  const handleSaveTier = async () => {
    setTierSaving(true); setDataError(null);
    const payload = { ...tierForm, display_order: Number(tierForm.display_order), price: Number(tierForm.price) };
    try {
      if (editingTier) {
        await api.admin.updateTier(editingTier.id, payload);
      } else {
        await api.admin.createTier(payload);
      }
      setShowTierForm(false);
      setEditingTier(null);
      fetchTiers();
    } catch (err) {
      reportError(err, 'Could not save this ticket tier.');
    } finally {
      setTierSaving(false);
    }
  };
  ...
  const deleteTier = async (tier: TicketTier) => {
    setDataError(null);
    try {
      await api.admin.deleteTier(tier.id);
      fetchTiers();
    } catch (err) {
      reportError(err, 'Could not delete this ticket tier.');
    }
  };

  const generateRaffle = async () => {
    setRaffleLoading(true); setDataError(null);
    try {
      const { visitors: entrants } = await api.admin.raffle();
      setRaffleVisitors(entrants);
      setShowRaffle(true);
    } catch (err) {
      reportError(err, 'Could not build the raffle list.');
    } finally {
  ...
  const saveEventDate = async () => {
    ...
    try {
      await api.admin.updateEventSettings({ event_date: eventDate, event_end_date: eventEndDate });
      setDateMessage(t.admin.dateSaved);
    } catch (err) {
      if (!auth.handleAuthFailure(err)) {
        setDateMessage(err instanceof ApiError ? err.message : 'Error saving date');
      }
    } finally {
      setDateSaving(false);
  ...
  const saveCapacity = async () => {
    setDateSaving(true); setDataError(null);
    try {
      await api.admin.updateEventSettings({ ground_capacity: capacityInput });
      fetchMetrics();
    } catch (err) {
      reportError(err, 'Could not save the ground capacity.');
    } finally {
      setDateSaving(false);
    }
  };
  ...
  // Session check: a stored token is revalidated against the API before the
  // dashboard or the login screen is shown, so a still-valid session does not
  // flash the sign-in form.
          ...
          <div className="glass-strong rounded-2xl p-8 border border-border/30 text-center">
            ...
            <form onSubmit={handleAuth} className="space-y-4 text-left">
              <div>
                ...
                <input type="password" value={password} onChange={(e) => { setPassword(e.target.value); auth.clearError(); }} autoFocus
                  className={`w-full px-4 py-3 rounded-xl bg-input border text-foreground placeholder:text-foreground/30 outline-none focus:ring-2 focus:ring-primary/30 ${auth.error ? 'border-destructive' : 'border-border/40 focus:border-primary/40'}`} placeholder="••••••••" />
                {auth.error && <p className={`mt-1.5 text-xs text-destructive ${isBn ? 'font-bengali' : ''}`}>{auth.error}</p>}
              </div>
              <button type="submit" disabled={auth.pending} className="w-full inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-gradient-gold text-white font-bold text-sm shadow-gold hover:shadow-gold-lg transition-all disabled:opacity-50">
                {auth.pending ? <Loader2 className="w-4 h-4 animate-spin" /> : <LogIn className="w-4 h-4" />} {t.admin.signInBtn}
              </button>
            </form>
          </div>
        </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      {/* API failures used to fail silently; they are now surfaced here. */}
      {dataError && (
        <div className="mb-6 flex items-start gap-2 px-4 py-3 rounded-xl bg-destructive/10 border border-destructive/30">
          <AlertCircle className="w-4 h-4 text-destructive shrink-0 mt-0.5" />
          <p className="text-xs text-destructive flex-1">{dataError}</p>
          <button onClick={() => setDataError(null)} className="text-xs text-destructive/60 hover:text-destructive">✕</button>
        </div>
      )}
      ...
      <div className={`glass-strong rounded-2xl p-6 border mb-8 ${crowd.isFull ? 'border-destructive/40' : 'border-border/30'}`}>
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <input type="number" value={capacityInput} min={1} onChange={(e) => setCapacityInput(parseInt(e.target.value) || 0)}
          ...
          {!loading && visitors.length > 0 && (
            <div className="p-4 border-t border-border/20 flex items-center justify-between">
              <p className="text-xs text-foreground/40">
                Page {page + 1} of {Math.max(1, Math.ceil(totalVisitors / pageSize))} · {totalVisitors} total
              </p>
              <div className="flex gap-2">
                ...
                <button onClick={() => setPage(page + 1)} disabled={(page + 1) * pageSize >= totalVisitors} className="px-3 py-1.5 rounded-lg glass border border-border/30 text-xs text-foreground/70 hover:border-primary/30 transition-all disabled:opacity-30">→</button>
              </div>
            </div>
          )}
        </div>
