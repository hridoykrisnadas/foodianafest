...
export default function Home() {
  ...
  // One request for the countdown date and all five carousels, replacing the
  // six round trips the browser used to make directly against the database.
  useEffect(() => {
    ...
    api.publicData
      ...
      .then((content) => {
        if (content.eventDate) setEventDate(content.eventDate);
        setGuests(content.guests as unknown as Guest[]);
        setAdvisors(content.advisors as unknown as Advisor[]);
        setManagement(content.management as unknown as ManagementMember[]);
        setSponsors(content.sponsors as unknown as Sponsor[]);
        setBrands(content.brandStalls as unknown as BrandStall[]);
      })
      .catch((error) => {
        if (error instanceof DOMException && error.name === 'AbortError') return;
        // The page is fully readable without live content, so fall back to the
        // static copy rather than blocking the landing page on the API.
